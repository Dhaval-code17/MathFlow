...
const GeometryBuilder = ({ onSolveCanvas }) => {
    ...
    const inputStyle = `w-16 bg-black/40 border border-[#00f5ff]/30 rounded text-center text-[#00f5ff] font-mono text-sm py-1 focus:outline-none focus:border-[#a855f7] focus:ring-1 focus:ring-[#a855f7] transition-all placeholder-white/20`;
        ...
        <div className="flex-1 flex flex-col h-full relative overflow-y-auto overflow-x-hidden">
            ...
            <div className="flex gap-2 mb-4 bg-black/20 p-1.5 rounded-lg border border-white/5">
                {SHAPES.map(s => (
                    <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-1 sm:gap-2 py-2 rounded-md text-xs sm:text-sm font-medium transition-all ${activeShape === s.id ? 'bg-[#00f5ff]/20 text-[#00f5ff] shadow-[0_0_10px_rgba(0,245,255,0.2)]' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
                    >
                        <s.icon size={16} />
            ...
            {activeShape === 'custom' ? (
                ...
                <>
                    ...
                    <div className="flex-1 relative bg-black/30 rounded-xl border border-white/5 flex items-center justify-center min-h-[300px]">
                    ...
                    >
                        {activeShape === 'triangle' && (
                            <div className="relative w-64 h-48 mt-8">
                                <svg width="100%" height="100%" viewBox="0 0 100 100" preserveAspectRatio="none">
                                    <polygon points="10,90 90,90 10,10" fill="rgba(0,245,255,0.1)" stroke="#00f5ff" strokeWidth="2" strokeLinejoin="round"/>
                                    {/* Right angle indicator */}
                                    <polyline points="10,80 20,80 20,90" fill="none" stroke="#00f5ff" strokeWidth="1.5" />
                                </svg>
                                {/* Side A (left) */}
                                <div className="absolute left-[-2rem] top-1/2 -translate-y-1/2 flex flex-col items-center">
                                    <span className="text-xs text-gray-400 font-mono mb-1">Side a</span>
                                ...
                                <div className="absolute bottom-[-3rem] left-1/2 -translate-x-1/2 flex flex-col items-center">
                                    ...
                                    <span className="text-xs text-gray-400 font-mono mt-1">Side b</span>
                                </div>
                                {/* Hypotenuse C */}
                                <div className="absolute top-1/2 left-1/2 translate-x-[-10px] translate-y-[-20px] flex flex-col items-center rotate-45 origin-center">
                                    <span className="text-xs text-[#a855f7] font-mono mb-1">Side c</span>
                        ...
                        {activeShape === 'rectangle' && (
                            <div className="relative w-72 h-40">
                                <svg width="100%" height="100%" viewBox="0 0 100 100" preserveAspectRatio="none">
                                    <rect x="10" y="10" width="80" height="80" fill="rgba(168,85,247,0.1)" stroke="#a855f7" strokeWidth="2" rx="4" />
                                    <line x1="10" y1="90" x2="90" y2="10" stroke="rgba(168,85,247,0.4)" strokeWidth="1" strokeDasharray="4,4" />
                                </svg>
                                {/* Height */}
                                <div className="absolute left-[-2rem] top-1/2 -translate-y-1/2 flex flex-col items-center">
                                    <span className="text-xs text-gray-400 font-mono mb-1">h</span>
                                ...
                                <div className="absolute bottom-[-3rem] left-1/2 -translate-x-1/2 flex flex-col items-center">
                                    ...
                                    <span className="text-xs text-gray-400 font-mono mt-1">w</span>
                                </div>
                                {/* Diagonal */}
                                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center bg-black/40 px-2 py-1 rounded">
                                    <span className="text-[10px] text-gray-400 font-mono mb-1">Diagonal</span>
                        ...
                        {activeShape === 'circle' && (
                            <div className="relative w-48 h-48">
                                <svg width="100%" height="100%" viewBox="0 0 100 100">
                                    <circle cx="50" cy="50" r="45" fill="rgba(255,0,255,0.1)" stroke="#ff00ff" strokeWidth="2" />
                                    <circle cx="50" cy="50" r="2" fill="#ff00ff" />
                                    <line x1="50" y1="50" x2="95" y2="50" stroke="#ff00ff" strokeWidth="1.5" strokeDasharray="4,2" />
                                </svg>
                                {/* Radius */}
                                <div className="absolute top-1/2 left-[60%] -translate-y-[120%] flex flex-col items-center bg-black/40 px-2 py-1 rounded">
                                    <span className="text-[10px] text-gray-400 font-mono">Radius (r)</span>
                                    <input type="number" value={data.r || ''} onChange={e => handleChange('r', e.target.value)} placeholder="r" className={`${inputStyle} border-[#ff00ff]/30 text-[#ff00ff] focus:ring-[#ff00ff]`} />
                                </div>
                            </div>
                        )}
                    </motion.div>
                ...
                <div className="absolute top-3 right-3 bg-black/60 backdrop-blur-md border border-white/10 rounded-lg p-3 min-w-[120px] pointer-events-none">
                    <h3 className="text-xs font-orbitron text-gray-400 mb-2 border-b border-white/10 pb-1">Properties</h3>
                    <div className="space-y-1 font-mono text-xs">
                        <div className="flex justify-between gap-4">
                            <span className="text-gray-500">Area</span>
                            <span className="text-[#00f5ff]">{data.area || '-'}</span>
                        </div>
                        {activeShape === 'circle' ? (
                            <div className="flex justify-between gap-4">
                                <span className="text-gray-500">Circum</span>
                                <span className="text-[#a855f7]">{data.c || '-'}</span>
                            </div>
                        ) : (
                            <div className="flex justify-between gap-4">
                                <span className="text-gray-500">Perimeter</span>
                                <span className="text-[#a855f7]">{data.perimeter || '-'}</span>
                            </div>
                        )}
                    </div>
                </div>
                ...
                {error && (
                    <div className="absolute bottom-3 left-1/2 -translate-x-1/2 bg-red-500/20 border border-red-500/50 text-red-200 text-xs px-3 py-1.5 rounded font-mono">
                        {error}
                    </div>
                )}
            </div>
            ...
            <div className="mt-4 flex gap-3">
                <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 text-gray-300 py-3 rounded-xl transition-all font-orbitron text-sm uppercase"
                >
                    <RotateCcw size={16} /> Clear
                </button>
                <button
                    onClick={handleSolve}
                    className="flex-[2] flex items-center justify-center gap-2 bg-gradient-to-r from-[#00f5ff]/20 to-[#a855f7]/20 hover:from-[#00f5ff]/40 hover:to-[#a855f7]/40 border border-[#00f5ff]/30 text-white py-3 rounded-xl transition-all font-orbitron tracking-wider shadow-[0_0_15px_rgba(0,245,255,0.2)]"
                >
